...
- fix read_query function
- 4/4 DONE function for querying and appending ticker table if necessary by querying both ticker and previous queries table
"""
...
async def read_query(
    ticker : str = "NFLX", start : str = "2011-11-01", end : str = "2012-02-01", interval : str = "1d"
    ):
    ...
    if stock_db_insp.has_table(ticker):
        # Need reflect in order for DB to be read and not return any errors server crashes without this line of code
        MetaData.reflect(stock_db_meta)
        ...
        if (earliest_start_date == latest_end_date and 
            earliest_start_date[TICKER_COL] != None and 
            latest_end_date[TICKER_COL] != None):
            return fetch_data_from_db(ticker, start, end)
            ...
            stock_data = yf.Ticker(ticker)
            stock_data = stock_data.history(start=start, end=latest_end_date[START_COL], interval=DAILY_CODE)
            stock_data = stock_data.iloc[:,OPEN_COLUMN:VOLUME_COLUMN]
            stock_data = stock_data.round(2)
            stock_data.reset_index(inplace=True)
            stock_data['Date'] = stock_data['Date'].dt.date
            stock_data.set_index('Date', inplace=True)
            stock_data.to_sql(name=ticker, con=stock_db_engine, if_exists='append')
            # Insert the ticker + dates so we have a historical timestamp of already known stored data
            stock_db_conn.execute(
                prev_queries_insert, 
                {
            ...
            stock_data = yf.Ticker(ticker)
            stock_data = stock_data.history(start=earliest_start_date[END_COL], end=end, interval=DAILY_CODE)
            stock_data = stock_data.iloc[:,OPEN_COLUMN:VOLUME_COLUMN]
            stock_data = stock_data.round(2)
            stock_data.reset_index(inplace=True)
            stock_data['Date'] = stock_data['Date'].dt.date
